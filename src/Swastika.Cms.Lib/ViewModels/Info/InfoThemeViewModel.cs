...
namespace Swastika.Cms.Lib.ViewModels.Info
{
       ...
       : ViewModelBase<SiocCmsContext, SiocTheme, InfoThemeViewModel>
    {
        ...
        public override RepositoryResponse<bool> SaveSubModels(SiocTheme parent, SiocCmsContext _context = null, IDbContextTransaction _transaction = null)
        {
            ...
            {
                FileRepository.Instance.SaveFile(Asset);
            }
            if (!InfoTemplateViewModel.Repository.CheckIsExists(
                t => t.FileName == SWCmsConstants.Default.DefaultTemplateLayout && t.TemplateName == Name
                , _context, _transaction))
            {
                InfoTemplateViewModel layout = new InfoTemplateViewModel(new SiocTemplate()
                {
                    Extension = SWCmsConstants.Parameters.TemplateExtension,
                    TemplateId = parent.Id,
                    TemplateName = Name,
                    FolderType = SWCmsConstants.TemplateFolder.Layouts,
                    FileFolder = this.TemplateFolder,
                    FileName = SWCmsConstants.Default.DefaultTemplateLayout,
                    ModifiedBy = CreatedBy,
                    Content = SWCmsConstants.Default.DefaultTemplateLayoutBody,
                    CreatedDateTime = DateTime.UtcNow
                });
                var saveResult = layout.SaveModel(false, _context, _transaction);
        ...
        public override async Task<RepositoryResponse<bool>> SaveSubModelsAsync(SiocTheme parent, SiocCmsContext _context = null, IDbContextTransaction _transaction = null)
        {
                ...
                , _context, _transaction))
            {
                ...
                var saveResult = await layout.SaveModelAsync(false, _context, _transaction);
                if (!saveResult.IsSucceed)
                {
                    Errors.AddRange(saveResult.Errors);
                }
