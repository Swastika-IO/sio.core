...
using Sio.Cms.Messenger;
using static Sio.Cms.Messenger.SioChatEnums;

namespace Sio.Cms.Messenger.Hubs
{
    public class SioChatHub : Hub
    {
        ...
        public async Task Join(HubRequest<MessengerConnection> request)
        {
            ...
            var user = new ViewModels.SioMessengerUsers.ConnectViewModel(request.Data);
            ...
            if (result.IsSucceed)
            {
                ...
                var getAvailableUsers = ViewModels.SioMessengerUsers.DefaultViewModel.Repository.GetModelListBy(u => u.Status == (int)SioChatEnums.OnlineStatus.Connected);
        ...
        */
        ...
        public override Task OnDisconnectedAsync(Exception exception)
        {
            ...
            var getUserDevice = ViewModels.SioMessengerUserDevices.DefaultViewModel.Repository.GetSingleModel(u => u.ConnectionId == Context.ConnectionId);
            if (getUserDevice.IsSucceed)
            {
                getUserDevice.Data.Status = DeviceStatus.Disconnected;
                var countOnlineDevice = ViewModels.SioMessengerUserDevices.DefaultViewModel.Repository.Count(d => d.UserId == getUserDevice.Data.UserId && d.DeviceId != getUserDevice.Data.DeviceId && d.Status == (int)DeviceStatus.Actived).Data;
                if (countOnlineDevice == 0)
                {
                    SendToAll(getUserDevice.Data.UserId, MessageReponseKey.MemberOffline, false);
                }
                var getUser = ViewModels.SioMessengerUsers.DefaultViewModel.Repository.GetSingleModel(u => u.Id == getUserDevice.Data.UserId);
