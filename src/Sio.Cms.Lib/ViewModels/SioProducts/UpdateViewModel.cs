...
namespace Sio.Cms.Lib.ViewModels.SioProducts
{
    public class UpdateViewModel
        : ViewModelBase<SioCmsContext, SioProduct, UpdateViewModel>
    {
        ...
        public List<SioPageProducts.ReadViewModel> Categories { get; set; }

        [JsonProperty("modules")]
        public List<SioModuleProducts.ReadViewModel> Modules { get; set; } // Parent to Modules
        ...
        public string TemplateFolderType
        {
            get
            {
                return SioEnums.EnumTemplateFolder.Products.ToString();
            }
        }
        ...
        public UpdateViewModel(SioProduct model, SioCmsContext _context = null, IDbContextTransaction _transaction = null) : base(model, _context, _transaction)
        {
        }
        ...
        public override void ExpandView(SioCmsContext _context = null, IDbContextTransaction _transaction = null)
        {
            ...
            StrNormalPrice = SioCmsHelper.FormatPrice(NormalPrice);
            StrDealPrice = SioCmsHelper.FormatPrice(DealPrice);
            StrImportPrice = SioCmsHelper.FormatPrice(ImportPrice);
            ...
            if (this.View == null)
            {
                this.View = SioTemplates.UpdateViewModel.GetDefault(SioEnums.EnumTemplateFolder.Products, Specificulture);
            }
        ...
        public override SioProduct ParseModel(SioCmsContext _context = null, IDbContextTransaction _transaction = null)
        {
            if (Id == 0)
            {
            ...
            if (ThumbnailFileStream != null)
            {
                ...
                {
                    SioConstants.Folder.UploadFolder, "Products", DateTime.UtcNow.ToString("dd-MM-yyyy")
                });
            ...
            ImportPrice = SioCmsHelper.ReversePrice(StrImportPrice);
        ...
        public override Task<RepositoryResponse<bool>> RemoveRelatedModelsAsync(UpdateViewModel view, SioCmsContext _context = null, IDbContextTransaction _transaction = null)
        {
            ...
            if (result.IsSucceed)
            {
                var navCate = _context.SioPageProduct.AsEnumerable();
            ...
            if (result.IsSucceed)
            {
                var navModule = _context.SioModuleProduct.AsEnumerable();
            ...
            if (result.IsSucceed)
            {
                var navMedia = _context.SioProductMedia.AsEnumerable();
            ...
            if (result.IsSucceed)
            {
                var navRelated = _context.SioProductMedia.AsEnumerable();
                ...
                foreach (var culture in getCultures.Data)
                {
                    result.Add(
                        new SupportedCulture()
                        {
                            ...
                            Id = culture.Id,
                            Lcid = culture.Lcid,
                            IsSupported = culture.Specificulture == initCulture || _context.SioProduct.Any(p => p.Id == Id && p.Specificulture == culture.Specificulture)
                        });
        ...
        public List<SioProductProducts.ReadViewModel> GetRelated(SioCmsContext context, IDbContextTransaction transaction)
        {
            var navs = SioProductProducts.ReadViewModel.Repository.GetModelListBy(n => n.SourceId == Id && n.Specificulture == Specificulture, context, transaction).Data;
            navs.ForEach(n => n.IsActived = true);
            return navs.OrderBy(p => p.Priority).ToList();
            //var query = context.SioProduct
            //    .Include(cp => cp.SioRelatedProductS)
            ...
            //result.ForEach(nav =>
            //{
            //    nav.IsActived = context.SioRelatedProduct.Any(
