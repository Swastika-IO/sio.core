...
namespace Sio.Cms.Lib.Repositories
{
    public class TemplateRepository
    {
        ...
        public bool DeleteTemplate(string name, string templateFolder)
        {
            ...
            {
                templateFolder,
                name + SioConstants.Folder.TemplateExtension
            });
        ...
        public List<TemplateViewModel> GetTemplates(string folder)
        {
            ...
            FileInfo[] Files = d.GetFiles(string.Format("*{0}", SioConstants.Folder.TemplateExtension)); //Getting cshtml files
            ...
            foreach (var file in Files)
            {
                using (StreamReader s = file.OpenText())
                {
                    result.Add(new TemplateViewModel()
                    {
                        FileFolder = folder,
                        Filename = file.Name,
                        Extension = SioConstants.Folder.TemplateExtension,
                        Content = s.ReadToEnd()
                    });
                }
            }
