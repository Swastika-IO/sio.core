...
// The Sio I/O Foundation licenses this file to you under the GNU General Public License v3.0 license.
...
namespace Sio.Cms.Lib.Repositories
{
    public class FileRepository
    {
        ...
        public FileViewModel GetUploadFile(string name, string ext, string FileFolder)
        {
            ...
            string folder = CommonHelper.GetFullPath(new string[] { SioConstants.Folder.UploadFolder, FileFolder });
         ...
         public bool CopyWebDirectory(string srcPath, string desPath)
        {
            ...
            {
                //Now Create all of the directories
                foreach (string dirPath in Directory.GetDirectories($"{SioConstants.Folder.WebRootPath}/{srcPath}", "*", SearchOption.AllDirectories))
                {
                    Directory.CreateDirectory(dirPath.Replace(srcPath, desPath));
                }

                //Copy all the files & Replaces any files with the same name
                foreach (string newPath in Directory.GetFiles($"{SioConstants.Folder.WebRootPath}/{srcPath}", "*.*", SearchOption.AllDirectories))
                {
                    File.Copy(newPath, newPath.Replace(srcPath, desPath), true);
                }
        ...
        {
            string fullPath = CommonHelper.GetFullPath(new string[] { SioConstants.Folder.UploadFolder, folder });
        ...
        public List<FileViewModel> GetWebFiles(string folder)
        {
            ...
            {
                DirectoryInfo path = new DirectoryInfo(dirPath);
                string folderName = path.ToString().Replace(@"\", "/").Replace(SioConstants.Folder.WebRootPath, string.Empty);
        ...
        public List<FileViewModel> GetFiles(SioEnums.FileFolder FileFolder)
        {
