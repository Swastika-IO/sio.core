'use strict';
app.controller('CustomerController', ['$scope', '$rootScope', '$routeParams', '$timeout', '$location', 'CustomerServices',
    function ($scope, $rootScope, $routeParams, $timeout, $location, customerServices) {
        $scope.request = {
            pageSize: '20',
            pageIndex: 0,
            status: '2',
        ...
        $scope.loadCustomer = async function () {
            ...
            var response = await customerServices.getCustomer(id, 'be');
        ...
        $scope.saveCustomer = async function (customer) {
            ...
            if (resp && resp.isSucceed) {
                ...
                $rootScope.isBusy = false;
                $scope.$apply();
                //$location.path('/backend/customer/details/' + resp.data.id);
            }
            else {
                if (resp) { $rootScope.showErrors(resp.errors); }
                $scope.$apply();
