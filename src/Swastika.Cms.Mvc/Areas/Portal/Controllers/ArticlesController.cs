...
namespace Swastika.Cms.Mvc.Areas.Portal.Controllers
{
    ...
    public class ArticlesController : BaseController<ArticlesController>
    {
        ...
        {
            RepositoryResponse<PaginationModel<ArticleListItemViewModel>> getArticles = 
                await ArticleListItemViewModel.Repository.GetModelListByAsync(
        ...
        public IActionResult Create(int? categoryId = null)
        {
            ...
            var vmArticle = new ArticleBEViewModel(article)
            {
                IsVisible = true,
                Domain = this._domain,
                CreatedBy = "admin",
        ...
        public async Task<IActionResult> Create(ArticleBEViewModel article, int? categoryId = null)
        {
            if (ModelState.IsValid)
            {
                //var vmArticle = new SWArticleBEViewModel<BackendArticleBEViewModel>(article);
        ...
        public async Task<IActionResult> Edit(string id = null, int? categoryId = null)
        {
            ...
            var article =  await ArticleBEViewModel.Repository.GetSingleModelAsync(
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(ArticleBEViewModel article, string id, int? categoryId = null)
        {
            if (id != article.Id)
            {
                return NotFound();
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    if (!ArticleBEViewModel.Repository.CheckIsExists(m => m.Id == article.Id))
                    {
                        return NotFound();
                    }
                    else
        ...
        {
            var article = await ArticleBEViewModel.Repository.RemoveModelAsync(m => m.Id == id && m.Specificulture == _lang);
            return RedirectToAction("Draft");
        }
    }
}
