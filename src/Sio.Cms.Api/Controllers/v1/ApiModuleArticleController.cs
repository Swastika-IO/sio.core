...
using System.Web;
using Sio.Cms.Lib.ViewModels.SioModuleArticles;
...
namespace Sio.Cms.Api.Controllers.v1
{
    ...
    public class ApiModuleArticleController :
        BaseGenericApiController<SioCmsContext, SioModuleArticle>
    {
        ...
        public async Task<RepositoryResponse<SioModuleArticle>> DeleteAsync(int moduleId, int articleId)
        {
        ...
        public async Task<ActionResult<JObject>> Details(string viewType, int? moduleId, int? articleId)
        {
            ...
            switch (viewType)
            {
                ...
                default:
                    ...
                    {
                        Expression<Func<SioModuleArticle, bool>> predicate = model => model.ModuleId == moduleId && model.ArticleId == articleId && model.Specificulture == _lang;
                        var portalResult = await base.GetSingleAsync<ReadViewModel>($"{viewType}_{moduleId}_{articleId}", predicate);
                        if (portalResult.IsSucceed)
                        {
                            portalResult.Data.Article.DetailsUrl = SioCmsHelper.GetRouterUrl("Article", new { portalResult.Data.Article.SeoName }, Request, Url);
                        }
                    ...
                    {
                        var model = new SioModuleArticle()
                        {
        ...
        public async Task<RepositoryResponse<SioModuleArticle>> SaveFields(int moduleId, int articleId, [FromBody]List<EntityField> fields)
        {
            if (fields != null)
            {
                var result = new RepositoryResponse<SioModuleArticle>() { IsSucceed = true };
            ...
            return new RepositoryResponse<SioModuleArticle>();
        }
            ...
            [FromBody] RequestPaging request)
        {
            ...
            Expression<Func<SioModuleArticle, bool>> predicate = model =>
