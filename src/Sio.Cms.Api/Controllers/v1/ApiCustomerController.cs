...
using Sio.Cms.Lib.ViewModels.SioCustomers;
...
namespace Sio.Cms.Api.Controllers.v1
{
    ...
    public class ApiCustomerController :
        BaseGenericApiController<SioCmsContext, SioCustomer>
    {
        ...
        [HttpGet, HttpOptions]
        [Route("delete/{id}")]
        public async Task<RepositoryResponse<SioCustomer>> DeleteAsync(int id)
        {
        ...
        public async Task<ActionResult<JObject>> Details(string viewType, int? id)
        {
            ...
            switch (viewType)
            {
                case "portal":
                    if (id.HasValue)
                    {
                        Expression<Func<SioCustomer, bool>> predicate = model => model.Id == id;
                        var portalResult = await base.GetSingleAsync<UpdateViewModel>($"{viewType}_{id}", predicate);
                        return Ok(JObject.FromObject(portalResult));
                    }
                    else
                    {
                        var model = new SioCustomer()
                        {
            ...
            [FromBody] RequestPaging request)
        {
            ...
            Expression<Func<SioCustomer, bool>> predicate = model =>
