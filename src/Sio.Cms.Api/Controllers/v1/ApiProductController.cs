...
using Sio.Cms.Lib.ViewModels.SioProducts;
...
namespace Sio.Cms.Api.Controllers.v1
{
    ...
    [Route("api/v1/{culture}/product")]
    public class ApiProductController :
        BaseGenericApiController<SioCmsContext, SioProduct>
    {
        public ApiProductController(IMemoryCache memoryCache, Microsoft.AspNetCore.SignalR.IHubContext<Hub.PortalHub> hubContext) : base(memoryCache, hubContext)
        {
        }
        ...
        [HttpGet, HttpOptions]
        [Route("delete/{id}")]
        public async Task<RepositoryResponse<SioProduct>> DeleteAsync(int id)
        {
        ...
        public async Task<ActionResult<JObject>> Details(string viewType, int? id)
        {
            ...
            switch (viewType)
            {
                case "portal":
                    if (id.HasValue)
                    {
                        Expression<Func<SioProduct, bool>> predicate = model => model.Id == id && model.Specificulture == _lang;
                        var portalResult = await base.GetSingleAsync<UpdateViewModel>($"{viewType}_{id}", predicate);
                        if (portalResult.IsSucceed)
                        {
                            portalResult.Data.DetailsUrl = SioCmsHelper.GetRouterUrl("Product", new { portalResult.Data.SeoName }, Request, Url);
                        }
                ...
                default:
                    if (id.HasValue)
                    {
                        ...
                        {
                            beResult.Data.DetailsUrl = SioCmsHelper.GetRouterUrl("Product", new { beResult.Data.SeoName }, Request, Url);
                        }
        ...
        [HttpPost, HttpOptions]
        [Route("save/{id}")]
        public async Task<RepositoryResponse<SioProduct>> SaveFields(int id, [FromBody]List<EntityField> fields)
        {
            if (fields != null)
            {
                var result = new RepositoryResponse<SioProduct>() { IsSucceed = true };
            ...
            return new RepositoryResponse<SioProduct>();
        }
            ...
            [FromBody] RequestPaging request)
        {
            ...
            Expression<Func<SioProduct, bool>> predicate = model =>
                    ...
                    if (mvcResult.IsSucceed)
                    {
                        mvcResult.Data.Items.ForEach(a =>
                        {
                            a.DetailsUrl = SioCmsHelper.GetRouterUrl(
                                "product", new { seoName = a.SeoName }, Request, Url);
                        });
                    }
