...
using System.Web;
using Sio.Cms.Lib.ViewModels.SioPortalPages;
...
namespace Sio.Cms.Api.Controllers.v1
{
    ...
    public class ApiPermissionController :
        BaseGenericApiController<SioCmsContext, SioPortalPage>
    {
        ...
        [HttpGet, HttpOptions]
        [Route("delete/{id}")]
        public async Task<RepositoryResponse<SioPortalPage>> DeleteAsync(int id)
        {
        ...
        public async Task<ActionResult<JObject>> Details(string viewType, int? id)
        {
            ...
            switch (viewType)
            {
                case "portal":
                    if (id.HasValue)
                    {
                        Expression<Func<SioPortalPage, bool>> predicate = model => model.Id == id;
                        var portalResult = await base.GetSingleAsync<UpdateViewModel>($"{viewType}_{id}", predicate);
                        return Ok(JObject.FromObject(portalResult));
                    }
                    else
                    {
                        var model = new SioPortalPage()
                        {
            ...
            [FromBody] RequestPaging request)
        {
            ...
            Expression<Func<SioPortalPage, bool>> predicate = model =>
        ...
        public async Task<RepositoryResponse<List<Lib.ViewModels.SioPortalPagePortalPages.ReadViewModel>>> UpdateNavInfos([FromBody]List<Lib.ViewModels.SioPortalPagePortalPages.ReadViewModel> models)
        {
            if (models != null)
            {
                RemoveCache();
                return await Lib.ViewModels.SioPortalPagePortalPages.ReadViewModel.UpdateInfosAsync(models);
            }
            else
            {
                return new RepositoryResponse<List<Lib.ViewModels.SioPortalPagePortalPages.ReadViewModel>>();
            }
        }
